...
```{r}
library(RCurl)
worlddata = subset(read.csv(text = getURL("https://raw.githubusercontent.com/cyadusha/Data_Extract_From_World_Development_Indicators/master/worlddata.csv"), sep = ","), select = c(Country.Name, X2005, X2013))
...
ggplot(data=dat.india.province) + geom_map(aes(fill=GINI, map_id=region),map=map.ind.regions1) + expand_limits(map.ind.regions1) + theme_bw() + scale_fill_gradientn(name="Coverage", colours = mycolors)
```
